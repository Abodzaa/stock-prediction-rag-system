...
- Confirm the News-Based RAG Answer block keeps the same dark-theme contrast and is visually prominent.
...
- Confirm the News-Based RAG Answer is visible near the top of the explainability panel without excessive scrolling after a run.
- Open the Explainability panel and confirm every citation tag jumps to the matching numbered source card.
- Confirm the News-Based RAG Answer uses only sources marked Used and falls back to the empty/no-source message correctly.
